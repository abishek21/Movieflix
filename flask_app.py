...
def index():
    if request.method == 'POST':
        ...
        if title:
            ...
            movie_name = string[:-1]
            # print(movie_name)
            search_url = 'https://www.imdb.com/search/title/?title=' + movie_name
            page = requests.get(search_url)
            soup = BeautifulSoup(page.text, 'html.parser')
            movies = soup.find("h3", class_="lister-item-header")
            # print(movies)
            for tag in movies.findAll('a', href=True):
                movie_id = str(tag['href'])

            movie_url = 'https://www.imdb.com' + movie_id + '?ref_=adv_li_tt'
            page = requests.get(movie_url)
            soup = BeautifulSoup(page.text, 'html.parser')
            summary = soup.find("div", class_="summary_text")
            summary = summary.text
            summary = summary.strip()
            return render_template('plot.html', summary=summary)
